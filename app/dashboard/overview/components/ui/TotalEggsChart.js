...
export function TotalEggsChart({ timeFrame }) {
  ...
  const generatePath = () => {
    ...
    const points = data.map((item, index) => {
      ...
      const y = padding + chartHeight - ((item.eggs || item.defects || 0) / maxEggs) * chartHeight
      return `${x},${y}`
    })
  ...
  return (
    <div className="relative">
      ...
      <div
        ref={chartRef}
        className="relative h-64 bg-gradient-to-br from-blue-50 to-indigo-100 rounded-lg overflow-hidden"
      >
        <svg
          width="100%"
          height="100%"
        ...
        >
          ...
          {[0, 0.25, 0.5, 0.75, 1].map((ratio) => (
            ...
            >
              {formatValue(Math.round(maxEggs * ratio))}
            </text>
          ))}
        </svg>
        ...
        {hoverData && (
          ...
          >
            ...
            <div className="text-lg font-bold text-blue-600">
              {(hoverData.eggs || hoverData.defects || 0).toLocaleString()} eggs
            </div>
          </div>
        )}
      </div>
